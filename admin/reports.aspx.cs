    ...
    public partial class reports : Page
    {
        ...
        protected void btnWeeklyReport_Click(object sender, EventArgs e)
        {
            ...
            if (projects.Count > 0)
            {
                ...
                CreateWeeklyPDFReport(projects, dt, project);
            }
            else
            {
        ...
        private void CreateWeeklyPDFReport(CSList<TimeProjectHours> projects, DateTime dt, TimeProjects projectDetails)
        {
            try
            {
                string templatePdfPath = Server.MapPath("PDFimages");
                ...
                byte[] b = WriteToPdfForWeekly(oldFile, projects, dt, projectDetails);
                if (b == null) return;
                HttpResponse response = HttpContext.Current.Response;
                response.Clear();
                response.ContentType = "application/pdf";
        ...
        public byte[] WriteToPdfForWeekly(string sourceFile, CSList<TimeProjectHours> projects, DateTime dt, TimeProjects projectDetails)
        {
            PdfReader reader = new PdfReader(sourceFile);
